...
fields = [
    'ImageNumber', 'AreaShape_Area', 'AreaShape_MajorAxisLength', 'AreaShape_MinorAxisLength'
]
...
def calc_mean(df, im_num, thing):
    """TODO make calc_mean docstring."""
    # so first i need to set the im and the thing
    # what do i want:
    #  0           1         2         3           4          5         6         7
    # ['Im', 'cellorcilia', 'MeanF1', 'Stdev F1', 'MeanF2', 'stdevf2', 'meanf3', 'stdevf3']
    cur_row = []
...
def convert_dict_to_csv(output):
    """TODO make convert_dict_to_csv docstring."""
    df = pd.DataFrame.from_dict(output)
    df.index = df.index + 1
    df.to_csv(
        path_or_buf=csv_path,
        header=[
            "Image Num", "Type of image", "Mean AreaShape_Area", "Stdev AreaShape_Area",
            "Mean AreaShape_MajorAxisLength", "Stdev AreaShape_MajorAxisLength",
            "Mean AreaShape_MinorAxisLength", "Stdev AreaShape_MinorAxisLength",
        ],
        index=False,
    )
