...
def main():
    ...
    parser.add_argument('-m', '--measurements', help='path to cellprofiler measurements', required=True)
    parser.add_argument('-c', '--c2c', help='path to c2c cilia/cent CSV if labeling cilia/cent', required=False)
    parser.add_argument('-o', '--output', help='output folder path', required=True)
    parser.add_argument('-g', '--images', help='folder with cellprofiler images path', required=True)
    parser.add_argument('-n', '--num', help='number of images to label, if specific number of im wanted', required=False)
    parser.add_argument('-h', '--channel', help='channel number for images (01 is nuclei, 02 is cilia, 03 is centriole)', required=True)
