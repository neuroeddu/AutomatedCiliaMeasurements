...
output_im_dir_path = r'/Users/sneha/Desktop/yann\ is\ cute.'  # thank u.
...
fields_polygons = [
    'ImageNumber', 'AreaShape_BoundingBoxMaximum_X', 'AreaShape_BoundingBoxMaximum_Y',
    'AreaShape_BoundingBoxMinimum_X', 'AreaShape_BoundingBoxMinimum_Y'
]
cilia_df_polygons = pd.read_csv(cilia_csv_path, skipinitialspace=True, usecols=fields_polygons)
...
for num in range(1, num_im+1):
    ...
    for cluster in cluster_li:
        ...
        for cilia in cluster:
            if not cur_polygon:
                ...
                cur_polygon = cascaded_union([cur_polygon, Polygon(cilia_vertices[cilia])])
    ...
    for multi_polygon in polygon_li:
        coords_list = []
        multi_polygon = [multi_polygon] if isinstance(multi_polygon, Polygon) else multi_polygon
