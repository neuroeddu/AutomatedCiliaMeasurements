...
centriole_csv_path = (
    "/Users/sneha/Desktop/mni/cilia_09:12:2021/im_output/MyExpt_Centriole.csv"
)
...
def cilia_to_line():  # big func that calls everything else
    ...
    if centriole:
        centriole_df = pd.read_csv(
            centriole_csv_path, skipinitialspace=True, usecols=fields
        )
        grouped_centriole = centriole_df.groupby(["ImageNumber"])

    for num in range(1, num_im + 1):
        cell_list, cilia_list, associate_list = make_lists_c2c(
            num, grouped_cell, grouped_cilia
        )
        im_path = make_paths(num, False)
        c2c_label(cell_list, cilia_list, associate_list, im_path, num)
        if centriole:
            centriole_list, cilia_list, associate_list = make_lists_c2c(
                num, grouped_centriole, grouped_cilia, True
            )
...
def make_paths(
    ...
    if label and not centriole_id:
        ...
        path = (
            output_im_dir_path + "CentrioleOverlay" + f"{num:04}" + "_LABELED_FULL.tiff"
        )
...
def c2c_label(cell_list, cilia_list, associate_list, im, num, centriole_id=False):
    ...
    for i, val in enumerate(associate_list):
        ...
        if (
            not cell_num == -2
        ):  # if cell num exists, find x and y coords of cilia and cell
