...
def plot(df):
    ...
    plt.scatter(df['Location_Center_X'].values.tolist(), df['Location_Center_Y'].values.tolist(), c=df['cluster'].values.tolist())
    plt.show()
...
df.drop(columns=['Location_Center_X','Location_Center_Y', 'Location_Center_Z', 'ObjectNumber'], inplace=True) # dropping object number bc it is arbitrary
...
for num in range(1, num_im+1):
    ...
    df.drop(columns=['Location_Center_X','Location_Center_Y', 'Location_Center_Z', 'ObjectNumber'], inplace=True) 
