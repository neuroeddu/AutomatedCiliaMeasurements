...
parser.add_argument(
    "-x", "--xmeans", help="whether xmeans should be included", required=True
)
parser.add_argument(
    "-p",
    "--pca_features",
...
# c2c_pairings.drop("ImageNumber", axis=1, inplace=True)
# print('here')
# # Merge so we get the list of all measurements we desire
# full_df = c2c_pairings.merge(measurements_cilia, on=["Cilia"])
...
# print('here')
# full_df['Centriole1'] = np.where(full_df['Cent1'].isnull(), 0, 1)
# full_df['Centriole2'] = np.where(full_df['Cent2'].isnull(), 0, 1)

# full_df = full_df.drop(['Cilia', 'Nucleus', 'Cent1', 'Cent2'])
