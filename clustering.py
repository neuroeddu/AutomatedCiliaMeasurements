...
tuned_parameters = [
    {"n_clusters":[3,5,1,50,7,9,2,4,25,6,8]} 
]
################################# TO CHANGE #################################
...
for num in range(1, num_im+1):
    ...
    full_df.drop(columns=['Location_Center_X','Location_Center_Y', 'Location_Center_Z', 'ObjectNumber'], inplace=True) 
    ...
    measurements_df=measurements_df[['Location_Center_X', 'Location_Center_Y', 'ObjectNumber']]
    full_df = cluster_map.merge(measurements_df, on=['ObjectNumber'])

    # Plot
    plt.scatter(full_df['Location_Center_X'].values.tolist(), full_df['Location_Center_Y'].values.tolist(), c=full_df['cluster'].values.tolist())
    plt.show()
