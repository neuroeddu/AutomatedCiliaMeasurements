...
def remove_noise(x_list, noise_list, num):
    ...
    :returns: List of x that only has the x that have been paired, List of indices with invalid
        centrioles skipped
    """
    valid_list = []
    true_idx_mapping = []
...
def combine_dicts(
    centriole_to_cell_no_dups, cilia_to_centriole_no_dups, num_im, real_idx_cent, cent_max
):
    """
    ...
    # Converting cilia to centriole into centriole to cilia for easier combination with the other
    # dictionary
...
def convert_format_output(c2c_output, num_im, cell_list, cilia_list):
    """
    Convert output from w.r.t centrioles to w.r.t cells & maintain 1:1 cell:cilia ratio.

    :param c2c_output: Combined dictionaries with all the information we want to store
    :param num_im: Image number
    :param cell_list: List of nuclei coordinates that we will use to calculate how far cilia are
        from nuclei
    :param cilia_list: List of cilia coordinates that we will use to calculate how far cilia are
        from nuclei
    ...
    for item in grouped.values:
        ...
        # If singleton cilia from aggregate(i.e. cell matched with one centriole), add
        # indiscriminately
        else:
            if isnan(item[3][0]):
                c2c_output_formatted[cell]['cilia'] = -1
...
def convert_dict_to_csv(c2c_output, output_path):
    ...
    df.to_csv(
        ...
        header=[
            "ImageNumber", "Nucleus", "PathLengthCentriole", "Centriole", "PathLengthCilia",
            "Cilia",
        ],
        index=False,
        float_format="%.10g"
        ...
        cilia_to_cent = [
            {
                'ImageNumber': num,
                'Cilia': x + 1,
                'Centriole': idx_dict[dict_cent['parent']],
                'path': dict_cent['path_length']
            } for x, dict_cent in enumerate(cilia_to_centriole) if dict_cent['parent'] != -1
        ]
        full_cent_to_cilia += cilia_to_cent

        # Make output and format it
        c2c_output_part = combine_dicts(
            centriole_to_cell, cilia_to_centriole, num, valid_cent_indices, len(centriole_list)
        )
        c2c_output_format, more_cilia_to_remove = convert_format_output(
            c2c_output_part, num, cell_list, cilia_list
        )

        # Make list of valid cilia and store
        _, valid_cilia_indices = remove_noise(
            cilia_list, cilia_to_remove.union(more_cilia_to_remove), num
        )
