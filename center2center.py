...
def combine_dicts(
    centriole_to_cell_no_dups,
    cilia_to_centriole_no_dups,
    num_im,
    real_idx_cent,
    cent_max,
):
    ...
    for idx, cilia_info in enumerate(cilia_to_centriole_no_dups):
        cent_to_cilia[cilia_info["parent"] - 1]["path_length"] = cilia_info[
            "path_length"
        ]
    ...
    for index, num in enumerate(real_idx_cent):
        ...
        c2c_output[index]["path_length_cell"] = centriole_to_cell_no_dups[num[1]][
            "path_length"
        ]
...
def main():
    ...
    for num in range(1, num_im + 1):
        ...
        valid_cent_coords, valid_cent_indices = remove_noise(
            centriole_list, cent_to_remove, num
        )

        # 1 index the valid centrioles and store them
        valid_cent_indices_1_idx = [
            [idx[0], (idx[1] + 1)] for idx in valid_cent_indices
        ]
        ...
        cilia_to_centriole, cilia_to_remove = nearest_child(
            valid_cent_coords, cilia_list, 1
        )
        cilia_to_cent = []
        ...
        cilia_to_cent = [
            ...
            for x, dict_cent in enumerate(cilia_to_centriole)
            if dict_cent["parent"] != -1
        ]
